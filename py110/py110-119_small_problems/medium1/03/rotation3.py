...
Explicit Rules: - function returns the maximum rotation of input integer:
    ...
    - function keeps rotating the resulting integer this way until it reaches
      the 2nd-last integer. Once this is complete, this is the maximum rotation
      of the integer.

Implicit Rules: - single digit input integers will be output as is - the last
digit doesn't need to be rotated; stop after rotating 2nd-last digit - if the
maximum rotation of a number has a leading zero or zeroes, the function
  will return that integer without the leading zero/es.
- Assume the function will only be passed valid integer values

Data Structure/s: - int - str

Algorithm: - coerce input to a string and assign it to str_input - if the length
of str_input is 1, return input - loop through a range backwards from the length
of str_input to 1
